...
const usdRState = atom({
  key: 'usdRState_key',
  default: 0,
});

const usdToEurSelectorRState = selector<number>({
  key: 'usdToEurSelectorRState_key',
  get: ({get}) => {
    let usdValue = get(usdRState);

    const commissionEnabled = get(commissionEnabledAtom);
    if (commissionEnabled) {
      const commissionValue = get(commissionAtom);
      usdValue = removeCommission(usdValue, commissionValue);
    }

    return usdValue * exchangeRate;
  },
  set: ({set, get}, eurNewValue) => {
    if (typeof eurNewValue === 'number') {
      let usdNewValue = eurNewValue / exchangeRate;

      const commissionEnabled = get(commissionEnabledAtom);
      if (commissionEnabled) {
        const commissionValue = get(commissionAtom);
        usdNewValue = addCommission(usdNewValue, commissionValue);
      }

      set(usdRState, usdNewValue);
    }
  },
});

export const Selectors = () => {
  const [usdValue, setUSDValue] = useRecoilState(usdRState);
  const [eurValue, setEURValue] = useRecoilState(usdToEurSelectorRState);
  return (
    <div style={{padding: 20}}>
      ...
      <InputStack>
        <CurrencyInput label="usd" amount={usdValue} onChange={(val) => setUSDValue(val)} />
        <CurrencyInput label="eur" amount={eurValue} onChange={(val) => setEURValue(val)} />
      </InputStack>
      <Commission />
    </div>
  );
